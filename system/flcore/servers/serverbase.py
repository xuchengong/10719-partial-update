...
class Server(object):
    def __init__(self, args, times):
        ...
        self.top_cnt = int(100 / args.eval_gap)
    ...
    def set_clients(self, clientObj):
        if self.dataset=='femnist':
            ...
            self.chosen_clients = np.random.choice(list(range(206)), self.num_clients, replace=False)
    ...
    def send_models(self):
        ...
        for client in self.clients:
            ...
            client.send_time_cost['total_cost'] += 2 * (time.time() - start_time) # why times 2?
    ...
    def aggregate_parameters(self):
        ...
        for w, client_model in zip(self.uploaded_weights, self.uploaded_models):
            for (k, v) in client_model.state_dict().items():
                new_weights[k] += v.clone() * w

        self.global_model.load_state_dict(new_weights)


    def save_global_model(self, result_path):
        model_path = os.path.join(result_path, f"server_{self.times}.pt")
        torch.save(self.global_model, model_path)
    

    def load_model(self):
        model_path = os.path.join("models", self.dataset)
        model_path = os.path.join(model_path, "server.pt")
    ...
    def evaluate_global(self, acc=None, loss=None):
        ...
        for c in self.clients:
            ct, ns, auc = c.test_metrics(self.global_model)
            tot_correct.append(ct*1.0)
            tot_auc.append(auc*ns)
            num_samples.append(ns)
        ids = [c.id for c in self.clients]
        ...
        num_samples, losses = [], []
        for c in self.clients:
            cl, ns = c.train_metrics(self.global_model)
            num_samples.append(ns)
            losses.append(cl*1.0)
        ids = [c.id for c in self.clients]
        ...
        print("Averaged Global Train Loss: {:.4f}".format(train_loss))
        print("Averaged Global Test Accurancy: {:.4f}".format(test_acc))
        print("Averaged Global Test AUC: {:.4f}".format(test_auc))
        # self.print_(test_acc, train_acc, train_loss)
        print("Std Global Test Accurancy: {:.4f}".format(np.std(accs)))
        print("Std Global Test AUC: {:.4f}".format(np.std(aucs)))
        return test_acc
    ...
    def call_dlg(self, R):
        ...
        for cid, client_model in zip(self.uploaded_ids, self.uploaded_models):
            ...
            for gp, pp in zip(self.global_model.parameters(), client_model.parameters()):
                ...
                origin_grad.append(gp.detach() - pp.detach())
