...
class FEMNIST_CNN(nn.Module):
    def __init__(self, in_features=1, num_classes=62, dim=512):
        ...
        self.dropout1 = nn.Dropout(0.25) # keep_prob=0.75
        self.fc1 = nn.Sequential(
            nn.Linear(dim, 128), 
            nn.ReLU(inplace=True)
        )
        self.dropout2 = nn.Dropout(0.5) # keep_prob=0.5
...
class EVEN_CNN(nn.Module):
    def __init__(self, in_features=1, num_classes=62, dim=128):
        super().__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_features,
                        16,
